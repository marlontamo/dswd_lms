...
class MenuController extends Controller
{
    ...
    //** Note: add custom name here : resources\lang\en\custom-menu.php */
    public function generatemenucontrol()
    {
        ...
        if (is_array(request()->input("arraydata"))) {
            foreach (request()->input("arraydata") as $value) {
                ...
                $menuitem = MenuItems::find($value["id"]);
                $menuitem->parent = $value["parent"] <= 0 ? $value["id"] : $value["parent"];
    ...
    // Remove Not Working
    public function generatemenucontrol_old(Request $request)
    {
        $main = NULL;
        $menu = Menus::find(request()->input("idmenu"));
        $depthfix = request()->input("arraydata");
        if (!empty($depthfix)) {
            foreach ($depthfix as $dk => $dv) {
                $id = $dv['id'];
                $depth = $dv['depth'];
                $parent = $dv['parent'];
                $sort = $dv['sort'];
                $cmenu = Menus::where('id', $id)->get();
                $cmenu->depth = $depth;
                $cmenu->parent = $parent;
                $cmenu->sort = $sort;
                $cmenu->save();
        ...
        $menu_bag_data = MenuItems::where('menus', '=', $menu)->post();

        $menu->name = str_slug(request()->input("menuname"));
        $menu->depth = request()->input("depth");
        $menu->save();
        $value = 0;
