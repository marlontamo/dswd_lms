...
trait FileUploadTrait
{
     ...
     * File upload trait used in controllers to upload files
     */
    public function saveMultipleFiles(Request $request, $downloadable_file_input = null, $model_type = null, $model = null)
    {
        ini_set('memory_limit', '-1');
        if (!file_exists(public_path('storage/uploads'))) {
            mkdir(public_path('storage/uploads'), 0777);
            ...
            if ($request->hasFile($key)) {
                ...
                if(is_array($request->file($key))){
                    if ($key == $downloadable_file_input) {
                        ...
                        foreach($request->file($key) as $singlekey => $singlevalue){
                            // Check file width
                            $extension = array_last(explode('.', $singlevalue->getClientOriginalName()));
                            $name = array_first(explode('.', $singlevalue->getClientOriginalName()));
                            $filename = time() . '-' . str_slug($name) . '.' . $extension;
                            $file = $singlevalue;
                            $image = Image::make($file);
                            if (!file_exists(public_path('storage/uploads/thumb'))) {
                        ...
                        $finalRequest = $finalRequest = new Request(array_merge($finalRequest->all(), [$key => $images]));
                    }
                }else{
                    if ($request->has($key . '_max_width') && $request->has($key . '_max_height')) {
                        // Check file width
     ...
     * File upload trait used in controllers to upload files
     */
    public function saveNonVideoFiles(Request $request, $path)
    {
        ini_set('memory_limit', '-1');
        if($path == ""){ $path = public_path('storage/uploads');}
