    ...
    <div class="container">
        ...
        <div class="blog-single-text">
            ...
            <div class="blog-details-content mt-3">
                <div class="video-container mb-5" data-id="{{$attachements->id}}">
                    @if($attachements->type == 'youtube')
                    <div id="player" class="js-player" data-plyr-provider="youtube" data-plyr-embed-id="{{$attachements->file_name}}"></div>
                    @elseif($attachements->type == 'vimeo')
                    <div id="player" class="js-player" data-plyr-provider="vimeo" data-plyr-embed-id="{{$attachements->file_name}}"></div>
                    @elseif($attachements->type == 'upload')
                    <video poster="" id="player" class="js-player" playsinline controls>
                        <source src="{{$attachements->url}}" type="video/mp4" />
                    </video>

                    @elseif($attachements->type == 'embed')
                    {!! $attachements->url !!}
                    @endif
                </div>
            </div>
            ...
            <div class="col-md-9">
                <div class="blog-details-content">
                    <div class="post-content-details">
                        ...
                        <div class="date-meta text-uppercase">
                            ...
                            <span><i class="fas fa-tag"><a href="{{route('blogs.category',['category' => $blog->category->slug])}}"> {{$blog->category->name}}</a></i></span>
                        </div>
                        <p>
                            {!! $blog->content !!}
                        </p>
                    ...
                    <div class="next-prev-post">
                        ...
                        <div class="next-post-item float-left">
                            <a href="{{route('blogs.index',['slug'=>$previous->slug.'-'.$previous->id ])}}"><i class="fas fa-arrow-circle-left"></i>Previous Post</a>
                        </div>
                        @endif

                        @if($next != "")
                        <div class="next-post-item float-right">
                            <a href="{{route('blogs.index',['slug'=>$next->slug.'-'.$next->id ])}}">Next Post<i class="fas fa-arrow-circle-right"></i></a>
                        </div>
                        @endif
                            ...
                            <div class="col-md-6">
                                <div class="blog-post-img-content">
                                    <div class="blog-img-date relative-position">
                                        <div class="blog-thumnile" @if($item->image != "") style="background-image: url({{asset('storage/uploads/blogs/'.$item->image)}})" @endif></div>
                ...
                <div class="blog-comment-area ul-li about-teacher-2">
                    <div class="reply-comment-box">
                        ...
                        <div class="teacher-faq-form">
                            <form method="POST" action="{{route('blogs.comment',['id'=>$blog->id])}}" data-lead="Residential">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="comment" required class="mb-0" id="comment" rows="2" cols="20"></textarea>
                        ...
                        @else
                        <a id="openLoginModal" class="btn nws-button gradient-bg text-white" data-target="#myModal"> @lang('labels.frontend.blog.login_to_post_comment')</a>
                        @endif
                    </div>
